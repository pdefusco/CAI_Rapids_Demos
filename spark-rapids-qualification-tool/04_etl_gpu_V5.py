...
spark = (
    ...
    # V5: 0.125 -> 0.25. Was 1/8 = 8 task slots competing for a GPU
    # that only runs 3 concurrent tasks (V3), causing scheduler churn
    # and extra host-memory pressure. 0.25 = 4 slots aligns closer to
    # the GPU's real concurrency and should relieve the memoryOverhead
    # squeeze introduced by V4's 4g pinned pool.
    ...
    # V4: 2g -> 4g. Faster host<->device transfers. Comes out of the
    # 8g executor memoryOverhead pool, so no container-size change.
    # Pairs with V3's concurrentGpuTasks bump: more concurrent GPU
    # tasks need more pinned host memory to stage inputs/outputs.
    .config("spark.rapids.memory.pinnedPool.size", "4g")
    .config("spark.rapids.sql.batchSizeBytes", "1g")
    # V3: 2 -> 3. Standard T4 sweet spot at 12g executor memory
    # plus 2g pinned pool; improves GPU utilization on the
    # withColumn-heavy analytical_transactions stage.
    ...
    # V2: 4g -> 1g. 4 GB per input partition under-parallelizes the
    # initial fact scan; 1 GB gives ~30-60 read partitions across
    # 8 executors x 8 cores, matching the GPU task slot count.
