...
| 3 | `04_etl_gpu_V3.py` | `spark.rapids.sql.concurrentGpuTasks` | `2` → `3` | Standard T4 sweet spot at 12g executor + 4g pinned. Improves GPU utilization on the withColumn-heavy stage. Kept in place for V4+V5 (works as a group with pinned-pool bump and task-slot rebalance). | **128.2s** | **161.8s** |  |
