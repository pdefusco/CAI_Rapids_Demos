...
TRANSACTION_TABLE = f"{DATABASE}.TRS_v26" # 1 B transactions
...
OUTPUT_TABLE = f"{DATABASE}.ETL_V9_RESULT_GPU"
...
# ============================================================
#
# This builder is the ONLY thing that differs from 02_etl_v9.py.
# Every table load, transformation, join, aggregation, and the final
# write below is copied verbatim from the CPU v9 script so that CPU
# vs. GPU timings are directly comparable.
#
# The configuration below combines:
#   1) The recommendations from NVIDIA's qualification tool:
#      qual_20260914214838_d8E33E9E/qual_core_output/tuning/
#          spark-94c75bfd78804baea75b0402d3ba224d.log
#   2) The GPU plumbing that 04_spark_rapids_etl_v4.py has already
#      proven necessary in this CAI workspace for the RAPIDS
#      Accelerator plugin to load and schedule GPU tasks (RAPIDS jar
#      extraClassPath, RapidsShuffleManager, GpuKryoRegistrator, GPU
#      discovery script).
#
# [FILL_IN_VALUE] placeholders from the tool have been resolved from
# the existing GPU-script defaults:
#   spark.executor.memory              -> 16g
#   spark.executor.memoryOverhead      -> 3g
#   spark.rapids.memory.pinnedPool.size-> 2g
#   spark.sql.adaptive.autoBroadcastJoinThreshold
#       -> disabled below via spark.conf.set(
#              "spark.sql.autoBroadcastJoinThreshold", -1)
#
# NOTE on executor count:
# The qualification tool suggested spark.executor.instances=63 and
# spark.dynamicAllocation.maxExecutors=62. Because each executor
# requests exactly one GPU (spark.executor.resource.gpu.amount=1),
# that would demand 63 GPUs concurrently. We deliberately keep the
# working 12-executor / 12-GPU shape used by the other GPU scripts.

RAPIDS_JAR_PATH = "/home/cdsw/.ivy2/jars/com.nvidia_rapids-4-spark_2.12-26.02.0.jar"

# Also produce a fresh event log so this GPU run can itself be fed
# into a second-pass profiler / qualification comparison.
...
spark = (
    ...
    .config("spark.executor.instances", "8")        # constrained to available CAI GPUs
    .config("spark.executor.cores", "8")             # tool recommendation
    .config("spark.executor.memory", "12g")           # FILL_IN default
    .config("spark.executor.memoryOverhead", "8g")    # FILL_IN default
    .config("spark.driver.cores", 4)
    .config("spark.driver.memory", "10g")

    # -- Network resilience for shuffle fetch under peer churn -------------
    .config("spark.network.timeout", "600s")           # default 120s
    .config("spark.rpc.askTimeout", "600s")            # default 120s
    .config("spark.executor.heartbeatInterval", "60s") # default 10s; must be < network.timeout
    .config("spark.shuffle.io.maxRetries", "10")       # default 3
    .config("spark.shuffle.io.retryWait", "15s")       # default 5s
    .config("spark.shuffle.io.connectionTimeout", "600s")  # default = network.timeout
    ...
    .config("spark.stage.maxConsecutiveAttempts", "8")     # default 4; more room for shuffle recovery

    # ------------------------------------------------------------
    # GPU scheduling
    # ------------------------------------------------------------
    .config("spark.executor.resource.gpu.amount", "1")
    .config("spark.task.resource.gpu.amount", 0.125)  # tool recommendation
    ...
    # ------------------------------------------------------------
    # RAPIDS SQL knobs (tool recommendations)
    # ------------------------------------------------------------
    .config("spark.rapids.sql.enabled", "true")
    .config("spark.rapids.filecache.enabled", "false")
    .config("spark.rapids.memory.pinnedPool.size", "2g")   # FILL_IN default
    ...
    # ------------------------------------------------------------
    # Spark SQL / AQE (tool recommendations)
    # ------------------------------------------------------------
    .config("spark.sql.shuffle.partitions", SHUFFLE_PARTITIONS)   # 1000
    ...
    # ------------------------------------------------------------
    # Event log (comparable to CPU v9)
    # ------------------------------------------------------------
    .config("spark.eventLog.enabled", "true")
    .config("spark.eventLog.dir", f"file://{EVENT_LOG_DIR_LOCAL}")

    # ------------------------------------------------------------
    # Storage (unchanged from v9 CPU)
    # ------------------------------------------------------------
    .config(
...
# Tool recommended spark.sql.adaptive.autoBroadcastJoinThreshold=[FILL_IN_VALUE].
# Match the existing GPU scripts and disable auto-broadcast at the SQL
...
print(f"[Spark-Rapids-ETL-v9 GPU T4] elapsed = {time.time()-t0:.1f}s")

_v9_gpu_elapsed = time.time() - _v9_gpu_start

print()
print("=" * 90)
print("SPARK RAPIDS ETL V9 COMPLETE")
print("=" * 90)
print(f"Source transactions : {TRANSACTION_TABLE}")
print(f"Output table        : {OUTPUT_TABLE}")
print(f"Shuffle partitions  : {SHUFFLE_PARTITIONS}")
print(f"GPU wall-clock      : {_v9_gpu_elapsed:.1f}s")
print()
